...
- **GET** → `meta`, `agents`, `usage_route`, `home`, `docs`, `config_route`, `workspace`, `terminal` (first match wins)
- **POST** → `agents`, `config_route`, `terminal`
...
| id | Module | Backend |
|---|---|---|
| `daily-coder` | `daily_coder.py` | HTTP proxy to `daily-coder serve` |
...
## Adding a third agent (checklist)
...
4. Restart the dashboard — no UI change required if you stick to unified run/approval shapes
