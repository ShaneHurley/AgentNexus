...
**Status:** Waves 0–2 implemented; workflow `2.1` adds `S_TRIVIAL`, context digests, last-chance frontier, schema `max_retries`, OpenAI tool/JSON wire improvements, and research early-stop. Retained as acceptance checklist and historical build plan.  
**Target package:** this repo (`daily-coder-ecosystem`).  
**Authority:** user build intent + `agent_orchestration_master_spec.md` Part D. Runtime truth: [`TECHNICAL.md`](TECHNICAL.md).
...
| Frontier escalation | Last-chance call when `cycles == max_repair_cycles` before rewind; never on exhausted halt |
| Sizer agent | Deterministic first; cheap role only when ambiguous; persists `trivial_signal` + `workflow` |
| Context digests | `plan_summary` / `implement_summary` / `verification_commands` lifted for downstream roles |
| Trivial short-circuit | `S_TRIVIAL` skips TEST_AUTHOR + DOCUMENT; keeps ALIGNMENT for acceptance |
...
6. **Budget check before every provider call**, including each research lane (per-call `reserve` under lock — parallel lanes cannot burst past the profile cap).
7. **Frontier:** at most one call per run, only via `authorize_frontier()`, only with a compressed decision brief — never the weak-model transcript. Invoked on the **last allowed repair cycle** so advice can feed the following retry; not spent on a terminal halt.
8. **Reasoning off by default** for lowest-tier roles (recon, implementer, documenter, alignment, sizer).
9. **Context packet shrinks**, not grows: each role gets only its required slice + task anchor + plan allowlist. Digests (`plan_summary`, `implement_summary`, …) are lifted for roles that declare them.
10. **Daily / session spend ceiling**: `max_spend_usd` and `max_tokens_per_day` in config; orchestrator refuses new runs when exceeded.
11. **Dry-run / mock is the default** until a human opts into a live provider with an explicit flag (`--live` or `provider != mock`).
...
| 0.4 | **Trivial short-circuit path** | One-line fixes skip RESEARCH/BRAINSTORM/frontier; **`S_TRIVIAL` also skips TEST_AUTHOR + DOCUMENT (keeps ALIGNMENT)** — done in workflow 2.1 | `router.py`, `workflow.py`, `state_machine.py` |
...
| 1.5 | **Wire frontier_advisor** via `escalation.py` | Only on qualifying triggers; count against `frontier_calls`. **Semantics (2.1):** call on last-chance repair cycle before rewind; never call-then-halt. | One expensive call max, logged |
