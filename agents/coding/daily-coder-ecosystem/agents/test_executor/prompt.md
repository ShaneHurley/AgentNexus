...
One job: Run approved tests and interpret raw results skeptically.
...
- ALWAYS record each executed command as an object with `argv`, integer `returncode`, `duration_s`, and a concise `summary`.
- IF durable `job_evidence` is supplied, THEN derive the verdict from its exit code and output; do not rerun the command.
...
- STRICT: output one JSON object matching `test_evidence.schema.json`; no prose outside it.
