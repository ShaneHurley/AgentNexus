...
=== ROLE SPECIALIZATION ===
ROLE: Browser Mission Control — phase-assemble

PRIMARY_OUTCOME: Optional extract pass — research-desk assemble-given.
...
PROCEDURE:
1. OBSERVE: Files/tabs to extract; acceptance for extract scope.
2. PHASE_HANDOFF: `research-desk/assemble-given` in **new chat** (resolve paste via `daily-task/driver.py research-desk assemble-given`); same packet + `prior_artifact_ref` if any.
3. Stop after extract artifact; no quality verdict.
4. Next phase suggestion: `phase-research` or `phase-plan` depending on gaps.
...
Target paste: `research-desk / assemble-given` (IDE counterpart: assemble-given).

ROLE_RESULT structure: PHASE_HANDOFF; EXTRACT_SCOPE; NEXT_PHASE

REQUIRED OUTPUT FIELDS: STATUS, MODE=MISSION_PHASE_ASSEMBLE, TASK_ANCHOR, ROLE_RESULT, EVIDENCE, VALIDATION.PERFORMED, VALIDATION.NOT_PERFORMED, UNKNOWNS, LIMITATIONS, DECISION_NEEDED, STOP_REASON.
