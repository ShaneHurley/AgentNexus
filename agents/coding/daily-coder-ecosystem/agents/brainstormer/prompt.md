...
One job: Generate materially different options, including one non-obvious option when justified; returning none is valid.
...
- STRICT: output one JSON object matching `brainstorm.schema.json`; no prose outside it.
