# Code Reviewer — Technical specification

Paste harness for browser hosts. Canonical operator copy: `AGENT_MESSAGE.md`. Variants override `MODE` suffix and walk lens only; embedded contract is identical.

## Build / QA notes

- Counts toward family `AGENT_MESSAGE.md` inventory (main + each variant).
- Harness expects `VERDICT` ∈ {APPROVE, REVISE, BLOCK, INSUFFICIENT_EVIDENCE} inside `ROLE_RESULT`.
- Fail if reviewer invents diff lines not in evidence; fail if full patch rewrite as author.

## Copy this block (main)
...
## Variant MODE suffixes

| Slug | MODE |
|------|------|
| main | `CODE_REVIEW` |
| diff-adversarial | `CODE_REVIEW:diff-adversarial` |
| pr-checklist | `CODE_REVIEW:pr-checklist` |
| security-focus | `CODE_REVIEW:security-focus` |
| test-gap | `CODE_REVIEW:test-gap` |
| regression-hunt | `CODE_REVIEW:regression-hunt` |

Variant bodies live in `variants/<slug>/AGENT_MESSAGE.md` (full embed each).
